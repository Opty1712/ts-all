...
Демо-монорепа к докладу на HolyJS «[Типизируем не только код: договорённости между слоями](https://holyjs.ru/talks/815e8c733791410cb50c184eecc3c97c/)».
...
## Экспериментальные демо-фичи

- `packages/app/src/router/experimentalRouter.tsx` - экспериментальный вариант роутинга, где `Router` выводится из того же дерева роутов, что и `APP_ROUTES`, а компонент страницы хранится прямо в route-конфиге как `Component`. Это демонстрационная идея для доклада, она еще не обкатана в продакшене.

## Ссылки из презентации

- [Слайды доклада «Типизируем не только код: договорённости между слоями»](<Типизируем не только код. Договоренности между слоями. Андрей Едунов. HolyJS. 15.05.2026.pdf>)
...
- [Статья про токены на Хабре](https://habr.com/ru/companies/vk/articles/1017912/)
...
- `perf-tests`
  Папка для замера перфоманса проекта
  1. `generate.mjs` - скрипт генерации большого количества компонентов, локалей и роутов
  2. `perf.mjs` - скрипт замера перфоманса проекта целиком, прогон через `tsc`
