...
- эксперимент:
  - `src/router/experimentalRouter.tsx` - экспериментальный кейс, где в route-конфиг добавлен `Component`, а `Router` строится автоматически из `APP_ROUTES`, то есть `Router` больше не надо писать руками и дублировать строки из `routes.ts`
  - отличие от обычного `routes.ts`: `routes.ts` описывает только контракт путей/параметров, а `Router.tsx` вручную связывает пути со страницами; `experimentalRouter.tsx` держит путь, параметры и `Component` в одном дереве
  - этот вариант пока не обкатан в продакшене и оставлен как экспериментальная фича демо-проекта
